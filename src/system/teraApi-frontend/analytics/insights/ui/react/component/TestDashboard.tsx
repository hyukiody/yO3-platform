...
const TestDashboard: React.FC = () => {
    ...
    try {
      ...
      if (response.ok) {
        try {
          ...
          if (isHealthy) {
            return { ...service, status: 'healthy', responseTime: Math.round(responseTime) };
          }
        } catch {
          console.warn(`Service ${service.name} returned valid status but unparseable JSON`);
          return { ...service, status: 'healthy', responseTime: Math.round(responseTime) };
        }
      }
      
      console.warn(`Service ${service.name} returned status ${response.status}:`, response.statusText);
      return { ...service, status: 'unhealthy', responseTime: Math.round(responseTime) };
    } catch (error) {
      const responseTime = performance.now() - startTime;
  ...
  useEffect(() => {
    // Skip health checks if user is not authenticated or not admin
  ...
  }, [isAuthenticated, user?.role, checkServiceHealth]);
