...
export default function ObjectDetectionMonitor() {
  ...
  const handleDetectionEvent = (event: DetectionEventDTO) => {
    ...
    const detectionObject: DetectionObject = {
      ...
      deepAnalysis: event.deepAnalysis,
  ...
  const getProtocolColor = (status?: string): string => {
    const colors: Record<string, string> = {
      none: '#4a4a4a',
      monitoring: '#4ECDC4',
      alert: '#FFE66D',
    ...
    return colors[status || 'none'] || '#4a4a4a';
  };
  ...
  return (
    <div className="object-detection-monitor">
      ...
      {/* Agent Insight Card - Agentic Vision Analysis */}
      {frameData && frameData.objects.some(obj => obj.deepAnalysis || obj.protocolStatus) && (
        <div className="agent-insight-card" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
            <span style={{ fontSize: '24px' }}>🤖</span>
            <h4 style={{ margin: 0, color: '#e94560' }}>Agent Insight</h4>
            {frameData.objects.some(obj => obj.protocolStatus === 'critical' || obj.protocolStatus === 'emergency') && (
              <span style={{
                background: '#e94560',
                color: '#fff',
                padding: '4px 12px',
          ...
          {frameData.objects.filter(obj => obj.deepAnalysis).slice(0, 3).map((obj) => (
            <div key={obj.id} style={{
              background: 'rgba(255,255,255,0.05)',
              borderRadius: '8px',
              padding: '12px',
              marginBottom: '8px',
              borderLeft: `4px solid ${getProtocolColor(obj.protocolStatus)}`,
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                <span style={{ color: '#fff', fontWeight: 'bold' }}>{obj.class}</span>
                {obj.protocolStatus && obj.protocolStatus !== 'none' && (
                  <span style={{
                    background: getProtocolColor(obj.protocolStatus),
                    color: '#fff',
                    padding: '2px 8px',
                    borderRadius: '4px',
                    fontSize: '11px',
