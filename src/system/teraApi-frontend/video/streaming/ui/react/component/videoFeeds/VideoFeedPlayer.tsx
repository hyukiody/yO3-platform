...
/**
 * Check if URL is a local webcam stream
 */
const isWebcamUrl = (url: string): boolean => url.startsWith('webcam://');

/**
 * Extract device ID from webcam:// URL
 */
const getWebcamDeviceId = (url: string): string => url.replace('webcam://', '');
 ...
 * 
 * NOW SUPPORTS: webcam:// URLs for local webcam streams
 */
export function VideoFeedPlayer({
  ...
  useEffect(() => {
    ...
    const handleEnded = () => {
      // Auto-loop if enabled (not applicable to webcam)
      if (loop && !isWebcamUrl(streamUrl)) {
        video.currentTime = 0
        video.play()
      }
    }
    ...
    // ═══════════════════════════════════════════════════════════════
    if (isWebcamUrl(streamUrl)) {
      setIsWebcam(true)
      const deviceId = getWebcamDeviceId(streamUrl)
      ...
      // ═══════════════════════════════════════════════════════════════
      // HTTP/RTSP STREAM HANDLING (existing logic)
      // ═══════════════════════════════════════════════════════════════
      setIsWebcam(false)
  ...
  useEffect(() => {
    ...
    // Connect to detection event stream with video element reference for webcam sync
    ...
    detectionEventService.subscribe(subscriberId, (event: DetectionEventDTO) => {
      // Only process events for this camera (or accept all in demo mode)
      const isDemo = event.cameraId?.startsWith('demo-') || event.cameraId?.startsWith('webcam-')
      if (event.cameraId !== cameraId && !isDemo) return
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        ...
        {isWebcam && <span style={{ ...styles.badge, background: 'rgba(0, 200, 255, 0.2)', borderColor: '#00c8ff', color: '#00c8ff' }}>🎥 WEBCAM</span>}
        {!isWebcam && loop && <span style={styles.badge}>🔄 LOOPING</span>}
        {isPlaying && <span style={{ ...styles.badge, background: 'rgba(255, 0, 0, 0.2)', borderColor: '#ff4444', color: '#ff6666' }}>● LIVE</span>}
        {enableDetection && isPlaying && (
          <span style={{ ...styles.badge, background: 'rgba(0, 255, 0, 0.2)', borderColor: '#00ff00', color: '#00ff00' }}>
            🤖 ML: {detections.length} obj
          </span>
        )}
      </div>
      ...
      <div style={styles.videoWrapper}>
        {isLoading && (
          <div style={styles.overlay}>
            <div style={styles.spinner}>{isWebcam ? '📷 Initializing camera...' : 'Loading video...'}</div>
          </div>
        )}
        ...
        <video
          ref={videoRef}
          style={isWebcam ? { ...styles.video, transform: 'scaleX(-1)' } : styles.video}
          controls={false}
          crossOrigin={isWebcam ? undefined : "anonymous"}
          playsInline
          muted={isWebcam}
        />
